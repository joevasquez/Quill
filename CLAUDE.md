...
33. **iOS orb-home redesign (2026-07, third wave)**: home is a capture launcher, not a note canvas (per the `Quill UI Update` handoff, `project/quill/ios/` — reskin only, no feature cuts).
    ...
    - **MCP OAuth at add time**: `MCPOAuthOrchestrator.advertisesOAuth(server:)` probes the RFC 9728 protected-resource metadata. Some servers (Dex) serve `initialize`/`tools/list` anonymously and only 401 actual tool calls, so the 401-triggered auto-sign-in never fired and the first voice action would fail. iOS `addServer` now signs in immediately when the probe succeeds; **macOS still uses the 401-only trigger** — mirror when touching the Mac Connections pane.
