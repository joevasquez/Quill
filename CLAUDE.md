...
21b. **Chip + Morph display mode (macOS)**: Third display mode (`HexSettings.displayMode: .chip`) that lives **in the menu bar**, per the Quill design handoff (`design_handoff_menu_bar_orb`). Architecture:
    ...
    - **Mode-name reveal**: `.onChange(of: store.selectedMode)` (fires on the cycle-mode hotkey and the menu Mode submenu — both funnel through `setMode`) shows the mode name (white) beside the glyph for ~1.6s, then collapses. `showModeReveal` grows the `NSStatusItem` via the `onDesiredLengthChanged` closure BEFORE animating the label in, and shrinks it only AFTER the fade-out (300ms > 250ms) so the label isn't clipped; rapid cycling cancels + resets the timer. Width is measured with `NSFont.systemFont(ofSize: 11, weight: .semibold)` to match the SwiftUI label. The controller's closure is guarded on `displayMode == .chip` so a stale reveal timer can't widen the static feather icon after the user leaves chip mode.
...
30. **iOS simplification + Mac-consistent UI pass (2026-07, second wave)**:
    - **Design tokens** (`HexCore/Models/QuillDesign.swift`): mode hues copied from the macOS orb (`QuillDesign.Hue` — keep in sync with `OrbHue` in OrbView.swift), semantic colors (`actionAccent` teal, `success` green, `mcpTile`), and `.quillCard()` — the one card treatment (frosted fill + hairline border). iOS action accents are now teal (was orange), matching the Mac.
