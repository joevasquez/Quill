...
10. **Dependent/chained Action steps**: The multi-action panel runs all parsed actions in parallel with no data flow between them, so "look up Joe's email in Dex, then draft him an email in Gmail" can't feed step 1's result into step 2. Needs: a `dependsOn` edge in the plan model, sequential execution for dependent steps, and an LLM "resolve" pass between steps that fills a later action's field from the prior result. This is the V2 of the flat multi-action panel — the workflow-engine evolution originally sketched for Hermes.

11. **LLM post-processing of MCP tool results**: MCP read/query tools return raw content (often JSON); the confirmation panel shows + copies it verbatim (`MultiActionConfirmationFeature` outputs). A small "answer from tool result" LLM pass would extract just what the user asked for ("his email address" → the email) and could paste it into the source app (plumb `sourceAppBundleID` into the panel) instead of only copying. Same primitive as #10's resolve step.

12. **MCP non-scalar arguments**: `ActionIntent.mcpArguments` is `[String: String]`, so MCP tools requiring array/object arguments can't be invoked. Extend the arg map to carry raw JSON values (and teach the planner prompt to emit them).

13. **Orb `SatelliteTarget` for MCP**: the Act-mode orb satellite ring is fed by the built-in `Integration.Identifier` enum, so connected MCP servers don't appear as tiles (they route by voice only). A `SatelliteTarget` abstraction (built-in integration OR MCP server) flowing through the ring + `fn+N` lock + keyword intuition would make MCP servers first-class on the orb.
