...
10. **LLM answer-extraction from MCP tool results**: MCP read/query results are now *formatted* for display by `MCPResultFormatter` (HexCore) — a schema-agnostic JSON→readable pass (name heading + `Label: value` lines, noise keys/image URLs dropped, pretty-print/raw fallbacks) applied when building `MultiActionConfirmationFeature` completion outputs (display + copy only; the raw result still feeds the resolve pass). The remaining opportunity is a *semantic* pass: a small "answer from tool result" LLM call that extracts just what the user asked for ("his email address" → the email) and pastes it into the source app (plumb `sourceAppBundleID` into the panel) instead of only copying. Same primitive as the dependent-step resolve pass (Implementation Detail #27).

11. **MCP non-scalar arguments**: `ActionIntent.mcpArguments` is `[String: String]`, so MCP tools requiring array/object arguments can't be invoked. Extend the arg map to carry raw JSON values (and teach the planner prompt to emit them).

12. **Orb `SatelliteTarget` for MCP**: the Act-mode orb satellite ring is fed by the built-in `Integration.Identifier` enum, so connected MCP servers don't appear as tiles (they route by voice only). A `SatelliteTarget` abstraction (built-in integration OR MCP server) flowing through the ring + `fn+N` lock + keyword intuition would make MCP servers first-class on the orb.

13. **Dependent steps on iOS + deeper chains**: dependent Action steps (Implementation Detail #27) are macOS-only — the shared planner prompt emits `dependsOn`, but only `MultiActionConfirmationFeature` runs the sequential resolve pass; iOS's `IOSSystemActionQueueExecutor` ignores the fields and runs steps independently. Also: the resolve pass is one level (a step consumes one dependency's raw text). A true DAG (a step consuming multiple prior results, or a producer feeding several consumers) would need multi-input resolve + topological scheduling.
