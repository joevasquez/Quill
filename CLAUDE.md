...
31. **Unified Connections (2026-07, third wave)**: native integrations and MCP servers share one user-facing concept — a *Connection*. MCP is an implementation detail.
    ...
    - **macOS**: `ConnectionsSectionView.swift` on the Integrations tab (below GoogleAccount + IntegrationsSectionView) = featured brands + custom server management (moved from AgentSectionView; `MCPServerRow`/`MCPServerSheet` moved with it).
