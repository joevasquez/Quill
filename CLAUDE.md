...
12. **Dependent steps on iOS + deeper chains**: dependent Action steps (Implementation Detail #27) are macOS-only — the shared planner prompt emits `dependsOn`, but only `MultiActionConfirmationFeature` runs the sequential resolve pass; iOS's `IOSSystemActionQueueExecutor` ignores the fields and runs steps independently. Same gap for the answer-extraction + paste flow (macOS panel only). Also: the resolve pass is one level (a step consumes one dependency's raw text). A true DAG (a step consuming multiple prior results, or a producer feeding several consumers) would need multi-input resolve + topological scheduling.
