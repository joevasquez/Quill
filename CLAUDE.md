...
7. **Keychain migration to `kSecUseDataProtectionKeychain`**: ✅ Done — `KeychainClient` now uses the Data Protection keychain with transparent legacy migration (see Lesson #9). The iOS `KeychainStore` was left as-is (iOS uses the DP keychain by default and doesn't surface the macOS password dialog).
