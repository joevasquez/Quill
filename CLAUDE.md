...
10. **LLM post-processing of MCP tool results**: MCP read/query tools return raw content (often JSON); the confirmation panel shows + copies it verbatim (`MultiActionConfirmationFeature` outputs). A small "answer from tool result" LLM pass would extract just what the user asked for ("his email address" → the email) and could paste it into the source app (plumb `sourceAppBundleID` into the panel) instead of only copying. Same primitive as the dependent-step resolve pass (Implementation Detail #27) — could reuse it to also surface a clean answer, not just feed the next step.
