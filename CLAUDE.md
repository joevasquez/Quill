...
21b. **Chip + Morph display mode (macOS)**: Third HUD skin (`HexSettings.displayMode: .chip`), a native SwiftUI port of the Quill design handoff (`design_handoff_menu_bar_orb`). A compact frosted **chip** shows the Quill **feather** at rest (slow breath) and **morphs** it into a mode-hued **orb** the moment capture starts (ink-drop overshoot — `.spring(response: 0.4, dampingFraction: 0.62)`); while listening the orb face becomes a 4-bar meter, and a **Corner Bloom** card drops below with the live transcript + Action-mode destination rows. Lives in `ChipMorphView.swift`, rendered inside the same `HUDPanel` as the other two modes via the branch in `TranscriptionView.body`. Reuses `OrbHue` + `Mode.orbHue` (promoted from `private` to internal in OrbView.swift so both orb-based modes are colour-identical) and `OrbView.intuitTarget` for Act routing. The `FeatherShape: Shape` ports the handoff's 3-stroke SVG (`M20 4C11 6 7 11 6 20 …`). **Design caveat:** the spec targets an `NSStatusItem` in the menu bar with the bloom anchored to the status-item frame; this implementation renders it as a floating panel like the other modes (geometry scaled ~1.8× from the 18px menu-bar spec). True menu-bar integration is a separate, larger change (custom layer-backed status-item view + panel re-anchoring), not a display-mode toggle.
