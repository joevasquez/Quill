...
- Search tools usually return SUMMARIES (id, name, title) — not detail fields like email addresses or phone numbers. When the user asks for a specific detail ("get Joe's email", "what's Sarah's phone number") and the server also exposes a get/detail tool (get_contact, get_record, …), emit TWO chained mcpCall steps: the search first, then the detail tool with dependsOn set and a resolveInstruction that fills the id argument from the search result. Do NOT answer a detail question with the search step alone.

  Example — chained MCP lookup for a detail field, assuming "Dex" has "search_contacts" (returns id/name summaries) and "get_contact" (takes contact_id, returns full details):
    Input: <transcript>get me the email contact information for Joe Vasquez from Dex</transcript>
    Output: {"actions":[{"actionType":"mcpCall","targetIntegration":"appleReminders","title":"Find Joe Vasquez in Dex","dueDate":null,"notes":null,"listName":null,"priority":null,"duration":null,"attendees":null,"recipient":null,"subject":null,"mcpServerName":"Dex","mcpTool":"search_contacts","mcpArguments":{"query":"Joe Vasquez"},"dependsOn":null,"resolveInstruction":null},{"actionType":"mcpCall","targetIntegration":"appleReminders","title":"Get Joe Vasquez's contact details","dueDate":null,"notes":null,"listName":null,"priority":null,"duration":null,"attendees":null,"recipient":null,"subject":null,"mcpServerName":"Dex","mcpTool":"get_contact","mcpArguments":{},"dependsOn":0,"resolveInstruction":"Fill the contact id argument (e.g. contact_id or id, matching the tool's schema) with Joe Vasquez's id from the search result"}]}
