...
//  or Dictate. Stateless and deterministic — easy to unit-test.
//

import Foundation

enum AutoModeClassifier {

  // MARK: - Live classification (partial transcript, during recording)

  /// Lightweight scan for the orb's live badge. No selection info
  /// available yet — just keyword analysis on the partial text.
  static func classifyPartial(_ transcript: String) -> TranscriptionIndicatorView.Mode {
  ...
  ) -> TranscriptionIndicatorView.Mode {
    let lowered = transcript.lowercased()
