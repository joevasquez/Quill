...
enum MCPOAuthClient {
  ...
  /// the RFC 7591 dynamic client registration the generic MCP OAuth flow
  /// relies on — but they accept standard Google bearer tokens, so they
  /// ride the app's own Google sign-in (mirrors `IOSMCPOAuthClient`).
  ...
  static func resolveAuthToken(for server: MCPServerConfig) async -> String? {
    if usesNativeGoogleAuth(server) {
      ...
      if await googleOAuth.isAuthorized(),
         let token = try? await googleOAuth.refreshIfNeeded() {
        return token
      }
    }
